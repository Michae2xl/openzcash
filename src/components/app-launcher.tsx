...
// The five cards shown in the fanned hero (à la the Claude template picker).
const FEATURED_IDS = [
  "transacoes",
  "zcg",
  "grants",
...
const FEATURED = FEATURED_IDS.map((id) => APP_BY_ID.get(id)).filter(
...
const ZID = ["z-0", "z-10", "z-20", "z-30", "z-40"];
...
export function AppLauncher({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  return (
    <div>
      {/* Fanned hero — the Claude-style template picker */}
      <p className="mb-6 text-center text-base font-medium text-stone-500">
        Start with an app…
      </p>
      <div className="flex justify-center overflow-x-auto px-4 pb-3 pt-1">
        <div className="flex">
          {FEATURED.map((app, i) => (
            <FanCard key={app.id} app={app} i={i} />
          ))}
        </div>
      </div>
      <div className="mt-6 text-center">
        ...
        >
          {expanded ? "…show less ↑" : "…or browse all apps →"}
        </button>
      </div>

      {/* Full app grid — revealed on demand */}
      {expanded ? (
          ...
          {isAdmin && ADMIN_APPS.length > 0 ? (
            <div className="mt-8 border-t border-stone-200 pt-6">
              <p className="mb-4 flex items-center gap-2 text-[11px] font-medium uppercase tracking-wider text-stone-500">
                <IconShield className="h-3.5 w-3.5 text-amber-600" /> Admin ·
                data management
              </p>
              <div className="grid grid-cols-3 gap-x-4 gap-y-6 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
                {ADMIN_APPS.map((app) => (
...
/** A single fanned card in the Claude-style hero: tinted panel + bold label. */
function FanCard({ app, i }: { app: AppDef; i: number }) {
  return (
    <Link
      href={app.href}
      aria-label={app.label}
      className={cn(
        "group relative block w-28 shrink-0 origin-bottom transition-transform duration-200 ease-out sm:w-40",
        i > 0 && "-ml-6 sm:-ml-8",
        TILT[i] ?? "rotate-0",
        ZID[i] ?? "z-0",
        "hover:z-50 hover:-translate-y-3 hover:rotate-0 hover:scale-[1.04]",
      )}
    >
      <div className="rounded-2xl border border-stone-200 bg-white p-2 shadow-lg shadow-stone-300/50 transition group-hover:shadow-2xl group-hover:shadow-stone-400/40">
        <div className="flex aspect-[5/4] items-center justify-center rounded-xl bg-[#efeae0]">
          <app.Icon className="h-9 w-9 text-stone-400 transition group-hover:text-amber-700 sm:h-12 sm:w-12" />
        </div>
        <p className="whitespace-nowrap py-2 text-center text-sm font-bold text-stone-900">
          {app.label}
        </p>
      </div>
    </Link>
